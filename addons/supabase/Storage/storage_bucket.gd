...
func download(object : String, to_path : String = "", private : bool = false, options: Dictionary = {}) -> StorageTask:
...
func create_signed_url(object : String, expires_in : int = 60000, options: Dictionary = {}) -> StorageTask:
...
func get_public_url(object: String, options: Dictionary = {}) -> String:
...
func _internal_process(_delta : float) -> void:
	...
	match _http_client.get_status():
		...
		HTTPClient.STATUS_CONNECTED:
			_http_client.request_raw(task._code, task._endpoint.replace(_config.supabaseUrl, ""), task._header, task.bytepayload)
		HTTPClient.STATUS_BODY:
			if _http_client.has_response() or _reading_body:
				_reading_body = true
				if _response_headers.is_empty():
					_response_headers = _http_client.get_response_headers()
				...
				if _http_client.get_status() != HTTPClient.STATUS_BODY:
					# INAYOS DITO: 4 arguments na lang, tinanggal ang 'null'
					task._on_task_completed(0, _response_code, _response_headers, _response_data)
			else:
				# INAYOS DITO: 4 arguments na lang
				task._on_task_completed(0, _response_code, _response_headers, _response_data)
